    ...
    if (storedUser) {
      try {
        ...
        if (parsedUser?.role === 'admin' || parsedUser?.role === 'volunteer') {
          router.replace(parsedUser.role === 'admin' ? '/admindb/dashboard' : '/volunteerdb/dashboard');
          return;
        }
      ...
      <aside className="w-60 bg-emerald-950 shrink-0 flex flex-col p-4">
        <div className="flex items-center gap-3 px-2 py-3 mb-8">
          ...
          <div className="leading-tight">
            <div className="text-white font-semibold text-sm">Donor Workspace</div>
            <div className="text-emerald-300 text-xs">Share surplus food</div>
          </div>
        </div>

        <nav className="flex flex-col gap-1">
          <Link
            href="/userdb/dashboard"
            aria-current={pathname === '/userdb/dashboard' ? 'page' : undefined}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${pathname === '/userdb/dashboard' ? 'bg-emerald-600 text-white' : 'text-emerald-300 hover:bg-emerald-900'}`}
          >
            <LayoutGrid className="w-4 h-4" />
            Overview
          </Link>
          <Link
            href="/userdb/donations"
            aria-current={pathname.startsWith('/userdb/donations') ? 'page' : undefined}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition-colors ${pathname.startsWith('/userdb/donations') ? 'bg-emerald-600 text-white' : 'text-emerald-300 hover:bg-emerald-900'}`}
          >
        ...
        <div className="mt-auto flex flex-col gap-1">
          <Link href="/userdb/donations" className="flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg bg-emerald-600 text-white text-sm font-semibold mb-4 hover:bg-emerald-700 transition-colors">
